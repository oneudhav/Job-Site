...
class Jobs extends Main_Controller {
	...
	public function index()
	{
		...
		$data['user_sidebar'] = 'jobseeker/user_sidebar';
	...
	//--------------------------------------------------------------
		...
		//--------------------------------------------------------------
		// search job by city
		public function location($title1)
		{
			$search['city'] = get_city_id($title1); // get city id by title

			// pagination
			$count = $this->job_model->count_all_search_result($search);
			$offset = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
			$url= base_url("jobs/location/".$title1);
	...
	{	
		...
		$data['jobscompany'] = $this->job_model->get_more_jobs_company($skills);

		$data['user_sidebar'] = 'jobseeker/jobs/job_detail_page'; // load sidebar for user
