...
class Auth_Model extends CI_Model{
	...
	// Email Verification	public function email_verification($code)	{		$this->db->select('firstname,lastname, email, token, is_active');		$this->db->from('xx_employers');		$this->db->where('token', $code);		$query = $this->db->get();		$result = $query->row_array();		$match = count($result);		if($match > 0){			$this->db->where('token', $code);			$this->db->update('xx_employers', array('is_verify' => 1, 'token'=> ''));			return $result;		}		else{			return false;			}	}
	//----------------------------------------------------------------------
	// login function
